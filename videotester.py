...
main = tkinter.Tk()
main.title("Facial Emotional Analysis") #designing main screen
...
def predict():
    ...
    text.insert(END,"Facial Expression Recognized as : "+names[predict])
    #antonyms_of_expression = get_antonyms(predict)
    #text.insert(END,"Facial Expression Recognized as : "+antonyms_of_expression)

    # Example usage with common facial expressions
    if predict == "happy":
    	antonyms_of_expression = get_antonyms(predict)
    	text.insert(END,"Good Keep it up : "+antonyms_of_expression)

    elif predict == "sad":
    	antonyms_of_expression = get_antonyms(predict)
    	print(f"Antonyms of '{expression}': {antonyms_of_expression}")
...
def get_antonyms(word):
    """
    Finds antonyms of a given word using NLTK's WordNet.
    Returns a set of antonyms or an empty set if none are found.
    """
    antonyms = set()
...
font = ('times', 13, 'bold')
title = Label(main, text='Facial Emotional Analysis')
